...
def evaluate_mcq(model, tokenizer, rows: List[Dict], device: str, max_length: int = 1024):
    total = 0
    correct = 0
    ...
    return {"num_examples": total, "accuracy": acc, "num_correct": correct}
...
def append_result_csv(out_csv: str, row: Dict):
    ...
    fieldnames = ["dataset", "split", "method", "model_dir", "base_model", "num_examples", "accuracy", "num_correct"]
...
def main():
    ...
    metrics = evaluate_mcq(model, tokenizer, rows, device, args.max_length)
